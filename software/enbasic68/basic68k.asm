...
ACIA_1   	= $A00000	     	 * Console ACIA base address
...
*	RTS
	    ...
	    LEA.L   ACIA_1,A0
TXNOTREADY
	    BTST.B  #5,LSR(A0)		* Test TXRDY bit
        BEQ     TXNOTREADY		* Until ACIA Tx ready
        MOVE.B  D0,THR(A0)
...
*	AND.B	#1, D0 		; mask D0 - RXF (active low)
*	BEQ	RETCHR
*	ANDI.B	#$FE,CCR	; clear the carry, flag we got no byte								
*	RTS
...
*	RTS
        ...
        BTST.B   #0,LSR(A0)     * Test RCV bit
        BEQ.S    RXNOTREADY     * Branch If ACIA Rx not ready
        MOVE.B   RHR(A0),D0     * Read character received
        MOVEM.L  (A7)+,A0/D1    * Restore working registers
		ORI.b	 #1,CCR	        * Set the carry, flag we got a byte
        RTS                     * Return
RXNOTREADY:
...
LAB_SMSG
	...
	dc.b	'for 68000 prototype, v1.0',$0D,$0A
